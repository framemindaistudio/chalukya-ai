...
export default function PlaceImage({ id, className = '', showCredit = false }: { id: string; className?: string; showCredit?: boolean }) {
  const c = C[id]
  if (!c) {
  ...
  return (
    <div className={`relative overflow-hidden bg-line ${className}`}>
      <img src={`/img/places/${id}.jpg`} alt={placeById[id]?.name.en ?? ''} loading="lazy" className="h-full w-full object-cover" />
      {showCredit && (
